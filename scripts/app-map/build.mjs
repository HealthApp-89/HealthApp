...
import { manifest, screensExempt } from './manifest.mjs';
...
const { tree, drift } = buildTree(manifest, facts, { exemptScreens: screensExempt });
...
for (const s of drift.unnarratedScreens) console.warn(`△ screen needs a plain-language description: ${s}`);
if (strict && (drift.undocumented.length || drift.stale.length || drift.unnarratedScreens.length)) {
  console.error(`Drift detected (${drift.undocumented.length} undocumented, ${drift.stale.length} stale, ${drift.unnarratedScreens.length} unnarrated screens).`);
  process.exit(1);
}
