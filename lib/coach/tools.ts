  ...
  input_schema: {
    ...
    properties: {
      ...
      session_prescriptions: {
        type: "object",
        description: "Mon-Sun map of full per-exercise prescriptions. Required. Keys are full weekday names (Monday-Sunday); values are arrays of PlannedExercise shapes. Generated by the prescription rule engine.",
...
export async function executeProposeWeekPlan(opts: {
  ...
  if (typeof i.session_prescriptions !== "object" || i.session_prescriptions === null) {
    return { ok: false, error: { error: "session_prescriptions must be an object" }, meta: { ms: Date.now() - t0, range_days: 0 } };
  }

  const payload = i as unknown as ProposeWeekPlanInput;

  // ── Server-side prescription validation (Task 17) ─────────────────────────
  // Build the context the validator needs: active block, previous week's
  // prescriptions, and per-primary maintenance baselines computed from the
  // user's recent clean working sets.
  ...
  const prevWeekStart = subtractDaysIsoLocal(payload.week_start, 7);
  ...
  const rirT = payload.rir_target ?? 2;
  ...
  // The validator wants a TrainingWeek-shaped argument; the propose payload
  // carries the subset it actually reads (week_start, session_plan,
  // research_phase, session_prescriptions). Cast through unknown to avoid
  // synthesizing the full row (no id/created_at/etc. at propose time).
  const validationErr = validateWeekPrescription({
    prescription: (payload.session_prescriptions ?? {}) as SessionPrescriptions,
    block: activeBlock,
    week: payload as unknown as TrainingWeek,
    prevWeek,
    ...
    .upsert(
      {
        ...
        session_prescriptions: p.session_prescriptions ?? null,
